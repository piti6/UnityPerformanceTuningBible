...
이 장에서는 AssetBundle에 대한 설정 및 구현 방침에 대해 설명합니다.

=={practice_assetbundle_granularity} AssetBundle의 세분화 정도

의존성 문제로 인해 AssetBundle을 얼마나 세분화할 것인지에 대해서는 신중하게 고려해야 합니다.
 ...
 * 동시에 사용되는 전제 에셋은 하나의 AssetBundle로 묶는다.
 * 여러 애셋에서 참조되는 애셋은 별도의 AssetBundle로 만든다.

완벽하게 제어하기는 어렵지만, 프로젝트 내에서 세분화에 대한 규칙을 정해두면 좋습니다.

=={practice_assetbundle_load} AssetBundle 로드 API

AssetBundle에서 애셋을 로드할 때 사용하는 API는 세 가지가 있습니다.
...
AssetBundle이 불필요해진 시점에서 언로드하지 않으면 메모리를 압박하게 됩니다,
이때 사용하는 API인 @<code>{AssetBundle.Unload(bool unloadAllLoadedObjects)}의 인수 @<code>{unloadAllLoadedObjects}는,
매우 중요하므로 개발 초기에 어떻게 설정할지 결정해야 한다.
이 인수가 true인 경우 AssetBundle을 언로드할 때 해당 AssetBundle에서 로드된 모든 애셋도 언로드한다.
false인 경우 애셋은 언로드되지 않습니다.

즉, 애셋을 사용하는 동안 AssetBundle도 계속 로드해야 하는 true는 메모리 부하가 높은 반면, 애셋을 확실히 파기할 수 있기 때문에 안전성이 높습니다.
반면 false의 경우 애셋을 다 로드한 시점에 AssetBundle을 언로드할 수 있기 때문에 그 순간 메모리 부하가 낮습니다,
하지만 다 사용한 애셋을 언로드하는 것을 잊어버리면 메모리 누수가 발생하거나 같은 애셋이 여러 번 메모리 상에 로드될 수 있기 때문에 적절한 메모리 관리가 필요하다.
일반적으로 엄격한 메모리 관리는 엄격해지기 때문에 메모리 부하가 여유가 있다면 @<code>{AssetBundle.Unload(true)}를 추천합니다.

=={practice_assetbundle_loadcount} 동시 로딩되는 AssetBundle 개수 최적화
...
파일 설명자란 파일을 읽고 쓸 때 OS 측에서 할당하는 작업용 ID를 말한다.
...
또한, 설령 상한에 도달하더라도 OS에 따라서는 일시적으로 상한을 높이는 것도 가능하므로 @<fn>{setrlimit}을 구현하는 것도 고려해 볼 수 있습니다.

#@# 각 플랫폼의 기본 파일 설명자 제한값을 조사하여 작성한다.

//footnote[setrlimit][리눅스/유닉스 환경에서는 setrlimit 함수를 사용하여 실행 시 제한값을 변경할 수 있다].

동시에 로드하는 AssetBundle이 많은 두 번째 문제는 Unity의 @<code>{PersistentManager.Remapper}의 존재입니다.
...
이러한 특성 때문에 동시 로딩 수에 비례하여 메모리를 압박하게 되므로 동시 로딩 수를 줄이는 것이 중요해집니다.

따라서 @<code>{AssetBundle.Unload(true)} 정책으로 운영할 경우, 동시 로드하는 AssetBundle은 최대 150~200개 정도를 기준으로 한다,
@<code>{AssetBundle.Unload(false)} 정책으로 운영할 경우 최대 150개 이하를 기준으로 하면 됩니다.
